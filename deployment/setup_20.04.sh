...
function setup_chatwoot() {
  ...
  git clone https://github.com/EduardoAldrimf/appchats.git
  cd chatwoot
  git checkout "$BRANCH"
  chmod -R 777 bin
  bundle
...
function setup_ssl() {
  ...
  curl ://ssl-config.mozilla.org/ffdhe4096.txt >> /etc/ssl/dhparam
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/EduardoAldrimf/appchats/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://raw.githubusercontent.com/EduardoAldrimf/appchats/master/deployment/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}
